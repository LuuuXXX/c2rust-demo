...
impl Feature {
    ...
    /// 2. Run `cargo new --lib`
    /// 3. Configure `Cargo.toml` (set crate-type, optionally add `cc` build-dependency)
    /// 4. Write `.cargo/config.toml` (sets `RUSTC_BOOTSTRAP=1` for `#![feature(linkage)]`)
    /// 5. Optionally generate `build.rs` for C coverage instrumentation
    /// 6. Generate per-file Rust scaffolding
    /// 7. Write `rust/src/lib.rs` and `lib.normalized`
    pub fn init(&self, coverage: bool) -> Result<()> {
        ...
        self.configure_cargo_toml(coverage)?;
        println!("Crate type configured");

        self.generate_cargo_config()?;
        println!("Cargo config generated (.cargo/config.toml with RUSTC_BOOTSTRAP=1)");

        if coverage && !self.files.is_empty() {
            self.generate_build_rs()?;
            println!("build.rs generated for C coverage instrumentation");
        }
    ...
    /// - Sets `[lib] crate-type = ["staticlib"]` (adds `"rlib"` when `coverage` is true
    ///   so that `cargo test` can build a test harness for `cargo llvm-cov`).
    /// - Adds `[build-dependencies] cc = "1.0"` when `coverage` is true.
    fn configure_cargo_toml(&self, coverage: bool) -> Result<()> {
        ...
        if coverage {
            crate_type.push("rlib");
        }
        lib.insert("crate-type", Item::Value(crate_type.into()));

        if coverage {
            let build_deps = doc["build-dependencies"].or_insert(Item::Table(Table::new()));
            let build_deps = build_deps
                .as_table_mut()
                .ok_or_else(|| anyhow!("[build-dependencies] is not a table"))?;
            build_deps.insert("cc", toml_edit::value("1.0"));
        }
    ...
    /// Write `rust/.cargo/config.toml` setting `RUSTC_BOOTSTRAP=1`.
    ///
    /// The generated `lib.rs` always contains `#![feature(linkage)]` which
    /// requires nightly Rust or `RUSTC_BOOTSTRAP=1` on stable.
    fn generate_cargo_config(&self) -> Result<()> {
        let cargo_dir = self.root.join("rust/.cargo");
        fs::create_dir_all(&cargo_dir)
            .ctx(&format!("create_dir_all {}", cargo_dir.display()))?;
        let config = r#"# Auto-generated by c2rust-demo.
# Required because the generated lib.rs uses #![feature(linkage)].
[env]
RUSTC_BOOTSTRAP = "1"
"#;
        let config_path = cargo_dir.join("config.toml");
        fs::write(&config_path, config.as_bytes())
            .ctx(&format!("write {}", config_path.display()))
    }

    /// Parse a `.c2rust.opts` file into individual flag strings.
    ///
    /// The file format is a sequence of double-quoted tokens separated by
    /// whitespace, e.g. `"-I." "-Dfoo=1" "-std=c99"`.
    fn parse_opts(content: &str) -> Vec<String> {
    ...
    /// Collect (original_c_path, compiler_flags) for every captured `.c2rust` file.
    ///
    /// The original C path is derived by stripping the `"2rust"` suffix from
    /// the `.c2rust` filename and joining with the project root.
    fn collect_c_source_info(&self) -> Vec<(PathBuf, Vec<String>)> {
        let c_root = self.root.join("c");
        ...
        for file in &self.files {
            ...
            // Derive relative path under c_root
            let rel = match file_path.strip_prefix(&c_root) {
                Ok(r) => r,
                Err(_) => continue,
            };
            // Strip the trailing "2rust" to get the original relative C path
            let rel_str = match rel.to_str() {
                Some(s) => s,
                None => continue,
            };
            let orig_rel = match rel_str.strip_suffix("2rust") {
                Some(s) => s,
                None => continue,
            };
            let orig_c_path = project_root.join(orig_rel);

            // Read opts file: <file_path>.opts
            let opts_path = format!("{}.opts", file_path.display());
            let opts_content = fs::read_to_string(&opts_path).unwrap_or_default();
            let flags = Self::parse_opts(&opts_content);

            sources.push((orig_c_path, flags));
        }
        sources
    }

    /// Generate `rust/build.rs` that compiles the captured C sources via the
    /// `cc` crate.  When `CARGO_LLVM_COV` is set (by `cargo llvm-cov`), the
    /// compilation uses `clang` with `-fprofile-instr-generate -fcoverage-mapping`
    /// to produce LLVM coverage data for the C side.
    fn generate_build_rs(&self) -> Result<()> {
        let sources = self.collect_c_source_info();
        if sources.is_empty() {
            return Ok(());
        }

        let rust_dir = self.root.join("rust");
        let mut code = String::new();
        code.push_str("// Auto-generated by c2rust-demo init --coverage\n");
        code.push_str("// Compiles original C sources so that cargo llvm-cov can measure C-side coverage.\n");
        code.push_str("fn main() {\n");
        code.push_str("    let coverage = std::env::var(\"CARGO_LLVM_COV\").is_ok();\n");
        code.push_str("    let mut build = cc::Build::new();\n");
        code.push_str("    build.flag(\"-fno-builtin\");\n");
        code.push_str("    if coverage {\n");
        code.push_str("        build.compiler(\"clang\");\n");
        code.push_str("        build.flag(\"-fprofile-instr-generate\");\n");
        code.push_str("        build.flag(\"-fcoverage-mapping\");\n");
        code.push_str("    }\n");

        for (src_path, flags) in &sources {
            // Forward include paths, defines, un-defines, and a few safe flags.
            for flag in flags {
                    ...
                    || flag == "-fno-strict-aliasing"
                {
                    ...
                    code.push_str(&format!("    build.flag(\"{escaped}\");\n"));
                }
            }
            let path_str = src_path
                ...
                .replace('\\', "\\\\")
                .replace('"', "\\\"");
            code.push_str(&format!("    build.file(\"{path_str}\");\n"));
        }

        code.push_str("    build.compile(\"c_ffi\");\n");
        code.push_str("    println!(\"cargo:rerun-if-changed=build.rs\");\n");
        code.push_str("}\n");

        let build_rs = rust_dir.join("build.rs");
        fs::write(&build_rs, code.as_bytes())
            .ctx(&format!("write {}", build_rs.display()))
    }
